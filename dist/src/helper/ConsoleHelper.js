...
const GitHelper_1 = __importDefault(require("./GitHelper"));
const prompts_1 = __importDefault(require("prompts"));
class ConsoleHelper {
    static maxHeaderBranchLength = 16;
    static minFeatureBranchLength = 5;
    options;
    packageName;
    gitHelper;
     ...
     */
    plotSummaryToConsole(featureBranchSummary, environmentBranches) {
        ...
        let tableOutput = "";
        do {
            const tableData = featureBranchSummary.map(item => this.getTableRow(item, featureBranchWidth));
     ...
     */
    getTableRow(featureBranchData, maxFeatureBranchLength) {
        let featureBranch = `${featureBranchData.isCurrent ? "* " : ""}${featureBranchData.branch}`;
        featureBranch = this.shortenWithMiddleEllipsis(featureBranch, maxFeatureBranchLength);
        ...
        const rowArray = [
            ...
            featureBranchData.committedAt,
            featureBranchData.committedBy,
     ...
     */
    getColorizedValue(value, data) {
        if (data.isEnvironmentBranch) {
            return chalk_1.default.grey(value);
        }
            ...
            return chalk_1.default.magentaBright(value);
        }
        else {
            return chalk_1.default.whiteBright.bold(value);
        }
    }
    /**
     ...
     */
    plotFollowUp(featureBranchData) {
        ...
        if (canBeMergedBranches.length) {
            console.log(`${chalk_1.default.whiteBright.bold(canBeMergedBranches.length)} branches have not been merged to all specified branches`);
            ...
            if (eligibleForDeletionButCheckedOut) {
                warnCurrentBranch = chalk_1.default.whiteBright.bold(eligibleForDeletionButCheckedOut.branch);
            }
        }
        if (fullyMergedBranches.length) {
            console.log(`${chalk_1.default.green.bold(fullyMergedBranches.length)} branches appear to be fully merged and may be removed`);
            const eligibleForDeletionButCheckedOut = fullyMergedBranches.find(branch => branch.isCurrent);
            if (eligibleForDeletionButCheckedOut) {
                warnCurrentBranch = chalk_1.default.green.bold(eligibleForDeletionButCheckedOut.branch);
            }
        }
        if (orphanLocalBranches.length) {
            console.log(`${chalk_1.default.magentaBright.bold(orphanLocalBranches.length)} branches appear to be local orphans and may be removed`);
            const eligibleForDeletionButCheckedOut = orphanLocalBranches.find(branch => branch.isCurrent);
            if (eligibleForDeletionButCheckedOut) {
                warnCurrentBranch = chalk_1.default.magentaBright.bold(eligibleForDeletionButCheckedOut.branch);
            }
        }
        if (neverMergedBranches.length) {
            console.log(`${chalk_1.default.yellow.bold(neverMergedBranches.length)} branches appear to either a) not have been merged, or b) is still in early stages of development`);
            const eligibleForDeletionButCheckedOut = neverMergedBranches.find(branch => branch.isCurrent);
            if (eligibleForDeletionButCheckedOut) {
                warnCurrentBranch = chalk_1.default.yellow.bold(eligibleForDeletionButCheckedOut.branch);
            }
        }
        ...
        if (eligibleForDeletion.length) {
            if (this.options.cleanup) {
                ...
                console.log(`\nNB: You may run ${chalk_1.default.bold.cyan(command)} to interactively remove fully merged branches and/or orphan local branches`);
                if (warnCurrentBranch) {
     ...
     */
    async doCleanup(branchesToClean, cleanup) {
        const caution = new ascii_table3_1.AsciiTable3()
            .setStyle("unicode-round")
            .addRowMatrix([[chalk_1.default.italic("CAUTION: Although it may appear these branches are all safe to delete, it could well be that they are still being used and just not have been merged yet! Always contact the developer(s) first before actually deleting branches!")]])
     ...
     */
    getTableOutput(environmentBranches, rowArrays) {
        const table = new ascii_table3_1.AsciiTable3("Merged / unmerged branches")
    ...
    shortenWithMiddleEllipsis(value, maxLength) {
    ...
    getLongestOutputLineLength(output) {
        return output
            .split("\n")
            .reduce((longest, line) => {
